...
void
KeccakP1600_StaticInitialize(void) {
...
void
KeccakP1600_InitializeRoundConstants(void) {
...
void
KeccakP1600_InitializeRhoOffsets(void) {
...
void
KeccakP1600_OverwriteBytes(void *state, const unsigned char *data, unsigned int offset,
                           unsigned int length) {
  assert(offset < 200);
  assert(offset + length <= 200);
...
void
KeccakP1600_OverwriteWithZeroes(void *state, unsigned int byteCount) {
...
void
KeccakP1600_Permute_Nrounds(void *state, unsigned int nrounds) {
...
void
KeccakP1600_Permute_12rounds(void *state) {
#if (PLATFORM_BYTE_ORDER != IS_LITTLE_ENDIAN)
  tKeccakLane stateAsWords[1600 / 64];
#endif
...
void
KeccakP1600OnWords(tKeccakLane *state, unsigned int nrRounds) {
  unsigned int i;
...
void
KeccakP1600Round(tKeccakLane *state, unsigned int indexRound) {
#ifdef KeccakReference
...
#define ROL64(a, offset) \
  ((offset != 0) ? ((((tKeccakLane)a) << offset) ^ (((tKeccakLane)a) >> (64 - offset))) : a)
...
static void
iota(tKeccakLane *A, unsigned int indexRound) {
...
void
KeccakP1600_ExtractBytes(const void *state, unsigned char *data, unsigned int offset,
                         unsigned int length) {
  assert(offset < 200);
  assert(offset + length <= 200);
...
void
KeccakP1600_ExtractAndAddBytes(const void *state, const unsigned char *input, unsigned char *output,
...
void
KeccakP1600_DisplayRoundConstants(FILE *f) {
  unsigned int i;
...
void
KeccakP1600_DisplayRhoOffsets(FILE *f) {
  unsigned int x, y;
